...
export default function ConnectPage() {
  ...
  const handleDisconnect = async (platformId) => {
    if (!window.confirm(`Are you sure you want to stop syncing with ${platformId}? This will remove related data from your current GigScore calculation.`)) {
      return;
    }
    setDisconnecting(platformId);
    try {
      await apiRequest('/platforms/disconnect', {
        method: 'POST',
        body: JSON.stringify({ platform_name: platformId }),
      });
  ...
  return (
    <DashboardLayout>
      <div className="mb-10">
        ...
        <p className="text-gray-500 text-lg">Connect or disconnect your gig profiles to control your risk data flow.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {platforms.map((platform) => {
          ...
          const isDisconnecting = disconnecting === platform.platformId;

          return (
            <div 
              key={platform.platformId} 
              className={`bg-white rounded-2xl p-8 border border-gray-100 shadow-[0_4px_12px_rgba(0,0,0,0.03)] flex items-center justify-between transition-all hover:shadow-xl hover:shadow-black/[0.02] ${isConnected ? 'opacity-100 ring-1 ring-black/5' : 'opacity-80'}`}
            >
              ...
              {isConnected ? (
                ...
                >
                   <div className="text-[10px] font-black uppercase tracking-widest px-5 py-2.5 rounded-lg border border-gray-100 bg-gray-50 text-gray-400 group-hover:bg-red-50 group-hover:text-red-600 group-hover:border-red-100 transition-all flex items-center gap-2">
                     {isDisconnecting ? <Loader2 className="animate-spin" size={12} /> : (
                       <>
                         <Pause size={10} className="fill-current" />
                         <span className="group-hover:hidden">Syncing</span>
                         <span className="hidden group-hover:inline">Stop Sync</span>
                       </>
                     )}
                   </div>
                </button>
      ...
      <div className="mt-16 p-10 bg-gray-900 rounded-3xl text-white flex flex-col sm:flex-row items-center justify-between gap-8">
        <div className="space-y-2">
          <h3 className="text-2xl font-black">Privacy Control</h3>
          <p className="text-gray-400 font-medium">When you stop syncing, we immediately delete your platform-specific data from our signal cache.</p>
        </div>
